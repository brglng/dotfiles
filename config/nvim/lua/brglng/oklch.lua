...
function M.linear_rgb_to_oklab(lr, lg, lb)
    -- Convert to XYZ
    local x = 0.4122214708 * lr + 0.5363325363 * lg + 0.0514459929 * lb
    local y = 0.2119034982 * lr + 0.6806995451 * lg + 0.1073969566 * lb
    local z = 0.0883024619 * lr + 0.2817188376 * lg + 0.6299787005 * lb

    -- Intermediate values
    local l = (x * 0.2104542553 + y * 0.7936177850 + z * -0.0040720468) ^ (1.0 / 3.0)
    local m = (x * 0.7936177850 + y * -0.0040720468 + z * 0.2104542553) ^ (1.0 / 3.0)
    local s = (x * 0.0040720468 + y * 0.2104542553 + z * 0.7936177850) ^ (1.0 / 3.0)
...
function M.oklab_to_linear_rgb(L, a, b)
    ...
    -- XYZ coordinates
    local x = 4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s
    local y = -1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s
    local z = -0.0041960863 * l - 0.7034186147 * m + 1.7076147010 * s

    -- Convert to linear RGB
    local lr = 3.2409699419 * x - 1.5373831776 * y - 0.4986107603 * z
    local lg = -0.9692436363 * x + 1.8759675015 * y + 0.0415550574 * z
    local lb = 0.0556300797 * x - 0.2039769589 * y + 1.0569715142 * z
...
function M.oklch_to_linear_rgb(L, C, h)
    local a, b = M.oklch_to_oklab(L, C, h)
...
---@param t number [0.0, 1.0], t=0.0 returns the first color, t=1.0 returns the second color
---@return number, number, number
function M.interpolate_rgb(r1, g1, b1, r2, g2, b2, t)
...
function M.blend_rgb(fg_r, fg_g, fg_b, bg_r, bg_g, bg_b, opacity)
    ...
    -- For zero chroma result, hue doesn't matter, use foreground hue
    local h
    if C < 0.0001 then
        h = fg_h
    else
        -- Calculate the shortest path around the color wheel
        local delta_h = fg_h - bg_h
        if delta_h > 180 then
            delta_h = delta_h - 360
        elseif delta_h < -180 then
            delta_h = delta_h + 360
...
-- Convert a packed RGB number to separate RGB components
---@param int integer
---@return number, number, number
function M.int_to_rgb(int)
    -- Extract the individual RGB bytes using LuaJIT bit operations
    local r_byte = bit.band(bit.rshift(int, 16), 0xFF)
    local g_byte = bit.band(bit.rshift(int, 8), 0xFF)
    local b_byte = bit.band(int, 0xFF)
...
---@param rgb2 integer
---@return integer
function M.interpolate(rgb1, rgb2, t)
    local r1, g1, b1 = M.int_to_rgb(rgb1)
    local r2, g2, b2 = M.int_to_rgb(rgb2)
    local r, g, b = M.interpolate_rgb(r1, g1, b1, r2, g2, b2, t)
    return M.rgb_to_int(r, g, b)
end
...
function M.blend(fg, bg, opacity)
    local fg_r, fg_g, fg_b = M.int_to_rgb(fg)
    local bg_r, bg_g, bg_b = M.int_to_rgb(bg)
